...
if (!empty($objectList)) {
    foreach ($objectList as $object) {
        ...
        foreach ($categoryListByObj as $category) {
            $count++;
            $numOfHotel = $categoryObj->get_number_of_product_has_categoryId($category['id']);
            ...
            ?>
                                    <tr>
                                        ...
                                        <td class="catalog_operator" class="btn-group btn-group-sm p-0 m-0" role="button group" aria-label="operator button group">
                                            <button type="button" onclick="infoModalEditCategory('<?=$category['id']?>', '<?= $location_name?>', '<?=$category['catagory_name']?>')" class="edit_producttpye btn btn-sm btn-secondary" data-bs-toggle="modal" data-bs-target="#edit_staticBackdrop">
                                                SỬA
                                            </button>
